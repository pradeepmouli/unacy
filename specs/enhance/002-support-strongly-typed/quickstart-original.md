...
This guide shows you how to use strongly typed metadata with the unacy branded type system. Unacy uses compile-time branded types (`WithUnits<T, U, M>`) rather than runtime class instances, with metadata stored in the registry and accessed via the `UnitAccessor` pattern.
